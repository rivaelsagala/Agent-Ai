"""Background scheduler for periodic market news collection and automatic dispatching."""
import logging
...
from app.services.tools.news_tools import OFFICIAL_SOURCES, send_market_alert
...
def get_sent_urls() -> Set[str]:
    ...
    return {r.get("url").strip() for r in records if r.get("url")}
...
def check_and_dispatch_news():
    """Periodic job: fetch news from official sources and send alerts for new articles."""
    logger.info("Executing periodic news monitoring job...")
    ...
    logger.info(f"Found {len(new_items)} new article(s) to process.")

    # Process up to 5 newest articles per tick to avoid spamming rate limits
    for source_name, item in new_items[:5]:
        date_str = format_indonesian_datetime(item.published_at)
        title = f"[{source_name}] {item.title}"
        body = (
            f"📅 Tanggal & Waktu: {date_str}\n"
            f"🔗 Link: {item.canonical_url}\n\n"
            f"📝 Ringkasan: {item.source_excerpt[:300]}..."
        )
        ...
        if settings.NOTIFICATION_TELEGRAM_TO and settings.TELEGRAM_BOT_TOKEN:
            try:
                ...
                logger.info(f"Telegram alert result: {res}")
                sent_any = True
            except Exception as e:
                logger.error(f"Error sending Telegram alert for '{item.title}': {e}")

        # Record in sent_news store so it is not processed again
        now = datetime.now()
